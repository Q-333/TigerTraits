//debug
console.log('BOOT server.js file:', __filename);
...
// ---- DB setup (local-first) ----
const uri = process.env.MONGO_URI || 'mongodb://localhost:27017/tigertraits';
console.log('Resolved DB URI:', uri);

let _client;
let _db;

/**
 * Connect to Mongo once and reuse the handle.
 * Works with local Mongo (mongodb://localhost:27017/tigertraits)
 * and Atlas SRV/non-SRV URIs if you later swap MONGO_URI.
 */
async function connectToMongoDB() {
  ...
  // If db name is in the URI path, client.db() uses it (tigertraits).
  // Otherwise you can pass a name: _client.db('tigertraits')
  _db = _client.db();
  console.log('Connected to MongoDB:', uri.includes('localhost') ? 'local' : 'remote');
  return _db;
}
...
app.post('/api/login', async (req, res) => {
    try {
      console.log('Alias /api/login called');
...
// 4. alias
app.post('api/quizResults',authenticateJWT, async (req, res) => {
    try {
        const db = await connectToMongoDB();
        const quizResultsCollection = db.collection('quizResults');
